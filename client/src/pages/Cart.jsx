...
const Cart = () => {
    ...
    const { setCartItems, cartItems, removeFromCart, updateCartItem, navigate, getCartAmount, getCartCount, products, currency, axios, user } = useAppContext()
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses.map((address, index) => (
                                    <p key={index}
                                        onClick={() => { setSelectedAddress(address); setShowAddress(false) }} className="text-gray-500 p-2 hover:bg-gray-100">
                                        {address.street} ,{address.city} ,{address.state} ,{address.country} ,
                                    </p>
                                ))
                                }
                                <p onClick={() => navigate('/add-address')} className="text-greeen-600 text-center cursor-pointer p-2 hover:bg-green-600/10">
                                    Add address
                                </p>
                            </div>
                        )}
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-green-600 text-white font-medium hover:bg-green-700 transition">
