...
function AddProducts() {
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            fromData.append('productData', JSON.stringify(productData));
            for (let i = 0; i < files.length; i++) {
                fromData.append('images', files[i])
            }

            const { data } = await axios.post('/api/product/add', fromData);
            if (data.success) {
    ...
    return (
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                ...
                <button className="px-8 py-2.5 bg-green-600 cursor-pointer text-white font-medium rounded">ADD</button>
            </form>
        </div>
    )
}
